...
class CanPhoenix():
    ...
    def setOperatingMode(self, mode):
        """Sets Operating Mode (e.g., 0xA1 for V2G)"""
        data = [0x21, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, mode]
        self._send_command(0x24, data)

    def setDcDischargeLowerVoltageLimit(self, volts):
        mv = int(volts * 1000)
        v_bytes = mv.to_bytes(4, byteorder='big')
        data = [0x11, 0x32, 0x00, 0x00, v_bytes[0], v_bytes[1], v_bytes[2], v_bytes[3]]
        self._send_command(0x24, data)

    def setReactivePowerMode(self, mode):
        data = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, mode]
        self._send_command(0x24, data)

    def setReactivePower(self, power):
        p_bytes = int(power).to_bytes(4, byteorder='big')
        data = [0x00, 0x00, 0x00, 0x00, p_bytes[0], p_bytes[1], p_bytes[2], p_bytes[3]]
        self._send_command(0x24, data)
...
if __name__ == "__main__":
    print("BeagleBone Black Phoenix V2G Test")
    ...
    try:
        ...
        # STEP 1: Disable readiness
        print("STEP 1: Disabling operational readiness...")
        charger.stop()
        time.sleep(0.5)

        # STEP 2 & 3: Switch AC Grid
        print("STEP 2 & 3: [ACTION] Simulated delay for manual AC switch.")
        time.sleep(1.0)

        # STEP 4: Configure V2G Parameters
        print("STEP 4: Configuring V2G Modes and Safety Limits...")
        charger.setOperatingMode(0xA1) # On-Grid
        time.sleep(0.2)
        charger.setDcDischargeLowerVoltageLimit(170.0)
        time.sleep(0.2)
        charger.setReactivePowerMode(0xA2)
        time.sleep(0.2)
        charger.setReactivePower(0)
        time.sleep(0.2)

        # STEP 5: Enable readiness
        print("STEP 5: Enabling operational readiness...")
        charger.start()
        time.sleep(0.5)
        ...
        time.sleep(0.2)
        charger.setEvTargetCurrent(10.0)
        
        # Monitor Loop
        print("\nSequence Complete. Monitoring CAN bus for 10 seconds...")
        for i in range(10):
            v = charger.getEvsePresentVoltage()
            c = charger.getEvsePresentCurrent()
            print(f"Reading {i}: {v}V, {c}A")
            time.sleep(1.0)
        
        # Final Cleanup
        print("\nShutting down charger...")
        charger.stop()
        charger.close()
        print("Done.")
